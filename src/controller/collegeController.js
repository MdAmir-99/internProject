...
// <--------These functions for the validations---------------->
...
const createCollege = async (req, res) => {
  try {
    ...
    if (!validate(name)) {
      return res.status(400).send({ status: false, message: `College Name (${name}) is in invalid format !!` });
    }
    ...
    if (!validate(fullName)) {
      return res.status(400).send({ status: false, message: `College FullName (${fullName}) is in invalid format !!` });
    }
    ...
    if (!validate(logoLink)) {
      return res.status(400).send({ status: false, message: `Image Url (${logoLink}) is in invalid format !!` });
    }

    if (!clgLogoLink(logoLink)) {
      return res.status(400).send({status: false,message: `This Url '${logoLink}' is Invalid !!`});
    }

    // <============Validation Ends Here==============>

    //<------------Object Destructring and store value of isDeleted AQT input------->
    const collegeDetails = {name,fullName,logoLink};
...
const getCollegeDetails = async (req, res) => {
  try {
    ...
    if (!clgName(name))
      return res.status(400).send({status: false,message: `College Name '${name}' is in invalid !!`});

    // <-------check CollegeName is Present in DB or not----------->
    const collegeName = await collegeModel.findOne({ name });

    if (collegeName == null)
      return res.status(400).send({status: false,message: `No College Found with this name '${name}'`});
    ...
    if(internDetails.length == 0) 
      return res.status(400).send({status : false, message:`No Intern Found With This College (${name})`}); 
